...
object BitmapUtils {
    ...
    fun getBitmap(image: ImageProxy): Bitmap? {
        val frameMetadata: FrameMetadata = FrameMetadata.Builder().setWidth(image.width).setHeight(image.height)
    ...
    private fun getExifOrientationTag(
        ...
        imageUri: Uri
    ): Int { // We only support parsing EXIF orientation tag from local file on the device.
        // See also:
        ...
        try {
            ...
            Log.e(
                TAG, "failed to open file to read rotation meta data: $imageUri", e
            )
            return 0
        }
    ...
    private fun yuv420ThreePlanesToNV21(
        ...
        if (areUVPlanesNV21(yuv420888planes, width, height)) { // Copy the Y values.
            yuv420888planes[0].buffer[out, 0, imageSize]
            val uBuffer = yuv420888planes[1].buffer
            val vBuffer =
                yuv420888planes[2].buffer // Get the first V value from the V buffer, since the U buffer does not contain it.
            ...
            // Unpack Y.
            unpackPlane(yuv420888planes[0], width, height, out, 0, 1) // Unpack U.
            unpackPlane(yuv420888planes[1], width, height, out, imageSize + 1, 2) // Unpack V.
